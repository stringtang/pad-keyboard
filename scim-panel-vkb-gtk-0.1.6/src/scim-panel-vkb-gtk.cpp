...
////////////system 20120216 by wy 
#define	KB_CMD_SUPPORTED 2
#define KEYBOARDON 0
#define KEYBOARDOFF 1
...
//static char new_command[64][MAXLINE];
int childpid;
static void
signal_handler (int signo)
{
    ...
    {
        g_message ("Caught signal %d", signo);
    }
    if(signo == SIGPIPE)
    {
        g_message ("Caught signal SIGPIPE\n");
    }
}
...
setup_signals (void)
{
    ...
    action.sa_mask = mask;  //并不屏蔽什么信号
    action.sa_flags = 0;
    sigaction (SIGTERM,  &action, NULL);//软件终止
    sigaction (SIGINT,  &action, NULL); //接收到终端中断符号：ctrl+c
...
static void       slot_real_turn_on_english            (void);
...
static gboolean bottom_to_middle(void);
static gboolean middle_to_bottom(void);
static gboolean middle_to_bottom_is_chinese(void);
static gboolean move_up(void);
/////////////////////////////////////////////////////////////////////////////
...
gboolean panel_vkb_toggle_vkb_alone(PanelVkbDbusObj * dbusobj, GError **error)
{
	...
	if(_vkb_embedded)
	{
		...
		if(_vkb_is_alone)
		{
			...
			if(bottom == 0)
			gtk_window_move(GTK_WINDOW(_input_window), 0, gdk_screen_height() - _input_window_height + LOOKUP_TABLE_MIN_HEIGHT);
			else
			gtk_window_move(GTK_WINDOW(_input_window), 0, gdk_screen_height() - _input_window_height*2);

			is_chinese = 0;
    			_panel_agent->change_factory ("c6bebc27-6324-4b77-8ad4-6d41dcaf2e08");
			_vkb_is_alone = false;
			slot_real_turn_on_english();
		}
		else
		{
...
gboolean panel_vkb_change_inputmethod(PanelVkbDbusObj * dbusobj, GError **error)
{
	...
	gtk_widget_hide(_lookup_table_parent);
	if(bottom == 0)
	gtk_window_move(GTK_WINDOW(_input_window), 0, gdk_screen_height()- _input_window_height);
	else
	gtk_window_move(GTK_WINDOW(_input_window), 0, gdk_screen_height()- _input_window_height*2);
    	_panel_agent->change_factory ("c6bebc27-6324-4b77-8ad4-6d41dcaf2e08");
	slot_real_turn_on_english();
...
gboolean panel_vkb_move_panel(PanelVkbDbusObj * dbusobj, GError **error)
{
	if(bottom == 0)
	{
		g_timeout_add(4, (GSourceFunc)bottom_to_middle, NULL);
		bottom = 1;
	}
	else
	{
	    if(is_chinese == 0)
	    {
		g_timeout_add(4, (GSourceFunc)middle_to_bottom, NULL);
	    }
	    else
	    {
		g_timeout_add(4, (GSourceFunc)middle_to_bottom_is_chinese, NULL);
	    }
		bottom = 0;
	}
	return TRUE;
...
gboolean bottom_to_middle(void)
{
    static int i;
    int y;

     y = gdk_screen_height()-302;
     gtk_window_move(GTK_WINDOW(_input_window), 0,y-i);
     i = i+MOVE_STEP;
     if(i <= 302)
     {
     return TRUE;
...
gboolean middle_to_bottom(void)
{
     static int i;

     gtk_window_move(GTK_WINDOW(_input_window), 0, gdk_screen_height()-302*2+i);
     i = i+MOVE_STEP;
     if(i <= 302)
     {
     return TRUE;
...
gboolean middle_to_bottom_is_chinese(void)
{
     static int i;
...
gboolean move_up(void)
{
	static int i;
    gtk_window_move(GTK_WINDOW(_input_window), 0, gdk_screen_height()+302 -i);
...
if(_vkb_embedded)
{
	...
	_input_window = gtk_window_new (GTK_WINDOW_POPUP);
        ...
        gtk_window_set_keep_above(GTK_WINDOW(_input_window), TRUE);
	g_timeout_add(4, GSourceFunc(move_up), NULL);
...
static void
slot_real_turn_on_english (void)
{
    _toolbar_should_hide = false;
    _toolbar_hidden = false;
    _panel_is_on = true;
...
slot_real_turn_off (void)
{
    ...
    //ui_hide_vkb_window ();
   g_timeout_add(4, (GSourceFunc)move_down, NULL);
...
int main (int argc, char *argv [])
{
    ...
    std::cerr << "GTK Panel of SCIM  aa" << SCIM_VERSION << "\n\n";
